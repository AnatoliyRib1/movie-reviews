...
func (r *Repository) GetPaginated(ctx context.Context, movieID, userID *int, offset int, limit int) ([]*Review, int, error) {
	selectQuery := dbx.StatementBuilder.
		Select("id", "movie_id", "user_id", "rating", "title", "content", "created_at", "deleted_at").
		From("reviews").
	...
	for rows.Next() {
		var review Review
		if err = rows.Scan(&review.ID, &review.MovieID, &review.UserID, &review.Rating, &review.Title, &review.Content, &review.CreatedAt, &review.DeletedAt); err != nil {
			return nil, 0, apperrors.Internal(err)
		}
